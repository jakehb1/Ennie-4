...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const FollowUpScreen = ({ onPositive, onNeutral, onShare, pins, setPins }) => {
  const [selPin, setSelPin] = useState(pins[0]?.id || null);
  const [side, setSide] = useState("front");
  ...
  const handleSubmit = (positive) => {
    setWasPositive(positive);
    setSubmitted(true);
  };

  // After brief confirmation, route to share (positive) or home (not positive)
  useEffect(() => {
    if (!submitted) return;
    const dest = wasPositive ? onShareRef : onPositiveRef;
    const t = setTimeout(() => dest.current(), 2000);
    return () => clearTimeout(t);
  }, [submitted, wasPositive]);
    ...
    <>
      ...
      <ScreenWrap>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", animation: "slideUp .4s ease", textAlign: "center" }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: wasPositive ? T.greenDim : T.accentDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 16, border: `2px solid ${wasPositive ? T.green : T.accent}40` }}>{wasPositive ? "✨" : "✓"}</div>
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 6 }}>{wasPositive ? "Your results held!" : "Check-in recorded"}</h2>
          <p style={{ fontSize: 13, color: T.textMuted, maxWidth: 260 }}>{wasPositive ? "That's a confirmed outcome. Let's capture it." : "Thanks for logging your follow-up."}</p>
        </div>
      </ScreenWrap>
    </>
  );
    ...
    <>
      ...
      <ScreenWrap>
        <div style={{ animation: "slideUp .4s ease" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[{ label: "100% better", icon: "✅", v: "accent", fn: "pos" }, { label: "It didn't help", icon: "✕", v: "secondary", fn: "neut" }].map(r => <Btn key={r.label} variant={r.v} small onClick={r.fn === "pos" ? () => handleSubmit(true) : onNeutral} style={{ flex: 1 }}>{r.icon} {r.label}</Btn>)}
          </div>
          <Btn onClick={() => handleSubmit(pins.every(p => p.severity <= 2))} full>Submit check-in →</Btn>
        </div>
      </ScreenWrap>
    </>
  );
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
window.EnnieApp = function EnnieApp() {
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case "followUp": return <FollowUpScreen onPositive={() => go("caseHome")} onNeutral={() => go("noResult")} onShare={() => go("share")} pins={finalPins.length > 0 ? finalPins : [{ id: 1, x: 52, y: 55, side: "front", severity: 3 }]} setPins={setFinalPins} />;
